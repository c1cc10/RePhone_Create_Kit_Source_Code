...
void _UG_PutChar( char chr, UG_S16 x, UG_S16 y, UG_COLOR fc, UG_COLOR bc, const UG_FONT* font)
{
   ...
   if (font->p == 0) {
       extern void gui_draw_font( char chr, UG_S16 x, UG_S16 y, UG_COLOR fc, UG_COLOR bc, const UG_FONT* font);

       gui_draw_font(chr, x, y, fc, bc, font);
       return;
   }
...
#endif
   {
	   ...
	   if (font->font_type == FONT_TYPE_1BPP)
	   {
//		 UG_FillFrame(x, y, x+actual_char_width-1,y+font->char_height-1, bc);
#define GUI_OPTIMIZED   1
#if GUI_OPTIMIZED
            extern vm_graphic_frame_t g_frame;
            extern uint8_t g_gui_changed;
#endif
         ...
         for( j=0;j<font->char_height;j++ )
         {
           ...
           for( i=0;i<bn;i++ )
           {
             ...
             while (b)
             {
               ...
               {
#if GUI_OPTIMIZED
                // vm_graphic_draw_point(&g_frame, xo + k, yo);
                uint16_t *pbuf = (uint16_t *)g_frame.buffer;
                pbuf += yo * 240 + (xo + k);
                *pbuf = ((fc >> 8) & (0x1F << 11)) | ((fc >> 5) & (0x3F << 5)) | ((fc >> 3) & 0x1F);
#else
                    gui->pset(xo + k,yo,fc);
#endif
               }
...
void _UG_PutText(UG_TEXT* txt)
{
   ...
   char* str = txt->str;
   char* c = str;
   UG_S16 current_char_width;


   //if ( txt->font->p == NULL ) return;
   if ( str == NULL ) return;
   if ( (ye - ys) < char_height ) return;
   ...
   while( 1 )
   {
      ...
      else if ( align & ALIGN_H_CENTER )
      {
          ...
          while( (*c != 0) && (*c != '\n') )
          {
             ...
             sl++;
             if (txt->font->p == 0) {
                 vm_graphic_set_font_size(txt->font->char_height);
                 current_char_width = vm_graphic_get_character_width(*c) + 1;
             } else {
                 current_char_width = (txt->font->widths ? txt->font->widths[*c - txt->font->start_char] : char_width);
             }
             wl += current_char_width + char_h_space;
             c++;
          }
          wl -= char_h_space;
      ...
      while( (*str != '\n') )
      {
         ...
         if (txt->font->p == 0) {
             vm_graphic_set_font_size(txt->font->char_height);
              current_char_width = vm_graphic_get_character_width(chr) + 1;
         } else {
              current_char_width = (txt->font->widths ? txt->font->widths[chr - txt->font->start_char] : char_width);
         }
         if ((xp + current_char_width) > xe) {
             str -= 2;
             break;
         }

         _UG_PutChar(chr,xp,yp,txt->fc,txt->bc,txt->font);
         xp += current_char_width + char_h_space;
      }
      str++;
...
UG_WINDOW* UG_GetActiveWindow( void )
{
    return gui->active_window;
}

UG_WINDOW* UG_GetLastWindow( void )
{
    return gui->last_window;
}
...
UG_RESULT UG_ButtonCreate( UG_WINDOW* wnd, UG_BUTTON* btn, UG_U8 id, UG_S16 xs, UG_S16 ys, UG_S16 xe, UG_S16 ye )
{
   ...
   btn->bc = C_BUTTON_BC;
   btn->fc = wnd->fc;
   btn->abc = C_BUTTON_ABC;
   btn->afc = wnd->fc;
   btn->style = BTN_STYLE_2D | BTN_STYLE_TOGGLE_COLORS | BTN_STYLE_NO_BORDERS;
   ...
   if ( obj->state & OBJ_STATE_UPDATE )
   {
      if ( obj->state & OBJ_STATE_VISIBLE )
      {
         ...
         if ( (obj->state & OBJ_STATE_REDRAW) || (btn->state & BTN_STATE_ALWAYS_REDRAW) )
         {
            ...
            /* Draw button text */
            txt.a.xs = obj->a_abs.xs+d + btn->h_space;
            txt.a.ys = obj->a_abs.ys+d;
            txt.a.xe = obj->a_abs.xe-d - btn->h_space;
